...
router.get('/impre_paises', async (req, res) => {
    const fecha = obtenerFechaActual();
    const doc = new PDFDocument();

    const stream = fs.createWriteStream(`./impresiones/Impre_Paises-${fecha}.pdf`);
    doc.pipe(stream);
    ...
    try {
        ...
        // Obtener los datos de los países desde la base de datos
        const dataPaises = await obtenerDatosPaises();
        // Llamar a la función para crear la tabla en el PDF
        crearTablaPaises(doc, dataPaises);
...
function crearTablaPaises(doc, dataPaises) {
    ...
    const tableHeaders = ["ID País", "Nombre"];
    ...
    function drawTable() {
        ...
        for (let i = 0; i <= dataPaises.length + 1; i++) {
        ...
        for (let i = 0; i <= tableHeaders.length; i++) {
            doc.moveTo(100 + i * cellWidth, y)
                .lineTo(100 + i * cellWidth, y + (fontSize + cellPadding) * (dataPaises.length + 1))
                .stroke();
        }
        ...
        dataPaises.forEach((pais, index) => {
            const fillColor = rowColors[index % rowColors.length];
            doc.fillColor(fillColor);
            drawRow([pais.ID_Pais, pais.Nombre], y, false, 'black'); // Dibujar el resto de las filas sin negrita
            y += fontSize + cellPadding;
        });
    }
