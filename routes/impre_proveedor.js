...
router.get('/impre_proveedor', async (req, res) => {
    const fecha = obtenerFechaActual();
    const doc = new PDFDocument();

    const stream = fs.createWriteStream(`./impresiones/Impre_Proveedores-${fecha}.pdf`);
    ...
    try {
        ...
        const dataProveedores = await obtenerDatosProveedores();
        crearTablaProveedores(doc, dataProveedores);
...
function crearTablaProveedores(doc, dataProveedores) {
    const tableTop = 250;
    ...
    const tableHeaders = ["ID", "Nombre", "Dirección", "Nit", "Encargado", "Teléfono", "Email"];
    ...
    function drawTable() {
        ...
        for (let i = 0; i <= dataProveedores.length + 1; i++) {
            doc.moveTo(50, y + i * (fontSize + cellPadding))
               .lineTo(50 + tableHeaders.length * cellWidth, y + i * (fontSize + cellPadding))
               .stroke();
        }
        for (let i = 0; i <= tableHeaders.length; i++) {
            doc.moveTo(50 + i * cellWidth, y)
               .lineTo(50 + i * cellWidth, y + (fontSize + cellPadding) * (dataProveedores.length + 1))
               .stroke();
        }
        drawRow(tableHeaders, y, true, '#28A787');
        y += fontSize + cellPadding;
        dataProveedores.forEach((proveedor, index) => {
            const fillColor = rowColors[index % rowColors.length];
            doc.fillColor(fillColor);
            drawRow([proveedor.ID_Proveedor, proveedor.Nombre, proveedor.Dirección, proveedor.Nit, proveedor.Nombre_encargado, proveedor.Teléfono, proveedor.Email], y, false, 'black');
            y += fontSize + cellPadding;
        });
    }

    function drawRow(rowData, y, bold, backgroundColor) {
        rowData.slice(0, 7).forEach((cellData, index) => {
            const cellX = 50 + index * cellWidth + cellPadding;
