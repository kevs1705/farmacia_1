...
function obtenerDatosCategorias() {
    return new Promise((resolve, reject) => {
      const query = 'SELECT * FROM categorias_productos';
...
router.get('/impre_categoria', async (req, res) => {
    const fecha = obtenerFechaActual();
    const doc = new PDFDocument();
 
    const stream = fs.createWriteStream(`./impresiones/Impre_Categorias-${fecha}.pdf`);
    ...
    try {
        ...
        doc.moveDown(); // Espacio vertical
        const mensaje = await obtenerMensajeReporte(23);
        // Imprimir el mensaje en el documento PDF
        doc.fontSize(10).font("Helvetica").text(`${mensaje}`, { align: "justify" });
     
        // Obtener los datos de las categorías desde la base de datos
        const dataCategorias = await obtenerDatosCategorias();
        // Llamar a la función para crear la tabla en el PDF
        crearTablaCategorias(doc, dataCategorias);
...
function crearTablaCategorias(doc, dataCategorias) {
    ...
    const tableHeaders = ["ID Categoría", "Nombre"];
    ...
    function drawTable() {
        ...
        for (let i = 0; i <= dataCategorias.length + 1; i++) {
        ...
        for (let i = 0; i <= tableHeaders.length; i++) {
            doc.moveTo(100 + i * cellWidth, y)
               .lineTo(100 + i * cellWidth, y + (fontSize + cellPadding) * (dataCategorias.length + 1))
               .stroke();
        }
        ...
        dataCategorias.forEach((categoria, index) => {
            const fillColor = rowColors[index % rowColors.length];
            doc.fillColor(fillColor);
            drawRow([categoria.ID_Categoria, categoria.Nombre_Categoria], y, false, 'black'); // Dibujar el resto de las filas sin negrita
            y += fontSize + cellPadding;
        });
    }
