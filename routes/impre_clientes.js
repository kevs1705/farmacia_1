...
router.get('/impre_clientes', async (req, res) => {
    const fecha = obtenerFechaActual();
    const doc = new PDFDocument();
    const stream = fs.createWriteStream(`./impresiones/Impre_Clientes-${fecha}.pdf`);
    doc.pipe(stream);
    ...
    try {
        ...
        doc.moveDown(); // Espacio vertical
        const mensaje = await obtenerMensajeReporte(20);
        ...
        const dataClientes = await obtenerDatosClientes();
        // Llamar a la función para crear la tabla en el PDF
        crearTablaClientes(doc, dataClientes);
       
        // Obtenemos la posición actual del documento
        const yPos = doc.y;
...
function crearTablaClientes(doc, dataClientes) {
    ...
    const tableHeaders = ["ID Cliente", "Nombre", "Apellido", "Telefono", "CI", "Nit", "Gmail"];
    ...
    function drawTable() {
        ...
        for (let i = 0; i <= dataClientes.length + 1; i++) {
        ...
        for (let i = 0; i <= tableHeaders.length; i++) {
            doc.moveTo(100 + i * cellWidth, y)
               .lineTo(100 + i * cellWidth, y + (fontSize + cellPadding) * (dataClientes.length + 1))
               .stroke();
        }
        ...
        dataClientes.forEach((cliente, index) => {
            ...
            drawRow([
                ...
                cliente.Nit || '', 
                cliente.Gmail || ''
            ], y, false, 'black'); // Dibujar el resto de las filas sin negrita
            y += fontSize + cellPadding;
        });
    }
    ...
    function drawRow(rowData, y, bold, backgroundColor) {
        const cellWidth = (doc.page.width - 200) / tableHeaders.length; // Calcular el ancho de cada celda
        rowData.forEach((cellData, index) => {
            ...
            const text = (cellData !== null && cellData !== undefined) ? cellData.toString() : '';
            if (bold) {
                doc.font("Helvetica-Bold").fontSize(fontSize).fillColor('#fff').text(text, cellX, y + (fontSize + cellPadding) / 2, { width: cellWidth, align: 'center' });
            } else {
                doc.font("Helvetica").fontSize(fontSize).fillColor(textColor).text(text, cellX, y + (fontSize + cellPadding) / 2, { width: cellWidth, align: 'center' });
            }
        });
    }
