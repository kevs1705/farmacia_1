...
function obtenerDatosTipoPaciente() {
    return new Promise((resolve, reject) => {
        const query = 'SELECT * FROM tipo_paciente';
...
router.get('/impre_tipo_paciente', async (req, res) => {
    const fecha = obtenerFechaActual();
    const doc = new PDFDocument();
 
    const stream = fs.createWriteStream(`./impresiones/Impre_Tipo_Paciente-${fecha}.pdf`);
    ...
    try {
        ...
        doc.moveDown(); // Espacio vertical
        const mensaje = await obtenerMensajeReporte(24);
        // Imprimir el mensaje en el documento PDF
        doc.fontSize(10).font("Helvetica").text(`${mensaje}`, { align: "justify" });
 
        // Obtener los datos de tipo paciente desde la base de datos
        const dataTipoPaciente = await obtenerDatosTipoPaciente();
        // Llamar a la función para crear la tabla en el PDF
        crearTablaTipoPaciente(doc, dataTipoPaciente);
        ...
        // Manejo de errores
        console.error('Error al obtener los datos de tipo paciente:', error);
...
function crearTablaTipoPaciente(doc, dataTipoPaciente) {
    ...
    const tableHeaders = ["ID Tipo Paciente", "Nombre"];
    ...
    function drawTable() {
        ...
        for (let i = 0; i <= dataTipoPaciente.length + 1; i++) {
        ...
        for (let i = 0; i <= tableHeaders.length; i++) {
            doc.moveTo(100 + i * cellWidth, y)
                .lineTo(100 + i * cellWidth, y + (fontSize + cellPadding) * (dataTipoPaciente.length + 1))
                .stroke();
        }
        ...
        dataTipoPaciente.forEach((paciente, index) => {
            const fillColor = rowColors[index % rowColors.length];
            doc.fillColor(fillColor);
            drawRow([paciente.ID_Tipo_Paciente, paciente.Nombre], y, false, 'black'); // Dibujar el resto de las filas sin negrita
            y += fontSize + cellPadding;
        });
    }
