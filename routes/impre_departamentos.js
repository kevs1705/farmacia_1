...
// Función para obtener los datos de los departamentos desde la base de datos
...
// Ruta para generar e imprimir el PDF de los departamentos
router.get('/impre_departamentos', async (req, res) => {
    const fecha = obtenerFechaActual();
    const doc = new PDFDocument();

    const stream = fs.createWriteStream(`./impresiones/Impre_Departamentos-${fecha}.pdf`);
    ...
    try {
        ...
        const mensaje = await obtenerMensajeReporte(19);
        // Imprimir el mensaje en el documento PDF
        doc.fontSize(10).font("Helvetica").text(`${mensaje}`, { align: "justify" });
      
      
        // Obtener los datos de los departamentos desde la base de datos
        const dataDepartamentos = await obtenerDatosDepartamentos();
        // Llamar a la función para crear la tabla en el PDF
        crearTablaDepartamentos(doc, dataDepartamentos);
...
function crearTablaDepartamentos(doc, dataDepartamentos) {
    ...
    const tableHeaders = ["ID Departamento", "Nombre"];
    ...
    function drawTable() {
        ...
        for (let i = 0; i <= dataDepartamentos.length + 1; i++) {
        ...
        for (let i = 0; i <= tableHeaders.length; i++) {
            doc.moveTo(100 + i * cellWidth, y)
                .lineTo(100 + i * cellWidth, y + (fontSize + cellPadding) * (dataDepartamentos.length + 1))
                .stroke();
        }
        ...
        dataDepartamentos.forEach((departamento, index) => {
            const fillColor = rowColors[index % rowColors.length];
            doc.fillColor(fillColor);
            drawRow([departamento.ID_Departamento, departamento.Nombre], y, false, 'black'); // Dibujar el resto de las filas sin negrita
            y += fontSize + cellPadding;
        });
    }
