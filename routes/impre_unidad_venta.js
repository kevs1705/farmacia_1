...
function obtenerDatosUnidadesVenta() {
    return new Promise((resolve, reject) => {
        const query = 'SELECT * FROM unidad_venta';
...
router.get('/impre_unidad_venta', async (req, res) => {
    const fecha = obtenerFechaActual();
    const doc = new PDFDocument();
    const stream = fs.createWriteStream(`./impresiones/Impre_Unidades_Venta-${fecha}.pdf`);
    doc.pipe(stream);
    ...
    try {
        ...
        doc.fontSize(13).font("Helvetica-Bold").text(`REGISTRO DE UNIDADES DE VENTA`, 160, 130);
        ...
        const dataUnidadesVenta = await obtenerDatosUnidadesVenta();
        crearTablaUnidadesVenta(doc, dataUnidadesVenta);

        const yPos = doc.y;
        agregarFirmas(doc, yPos);

    } catch (error) {
        console.error('Error al obtener los datos de las unidades de venta:', error);
...
function crearTablaUnidadesVenta(doc, dataUnidadesVenta) {
    const tableTop = 250;
    ...
    const tableHeaders = ["ID Unidad Venta", "Nombre"];
    ...
    function drawTable() {
        ...
        for (let i = 0; i <= dataUnidadesVenta.length + 1; i++) {
        ...
        for (let i = 0; i <= tableHeaders.length; i++) {
            doc.moveTo(100 + i * cellWidth, y)
               .lineTo(100 + i * cellWidth, y + (fontSize + cellPadding) * (dataUnidadesVenta.length + 1))
               .stroke();
        }
        drawRow(tableHeaders, y, true, '#28A787');
        y += fontSize + cellPadding;
        dataUnidadesVenta.forEach((unidad, index) => {
            const fillColor = rowColors[index % rowColors.length];
            doc.fillColor(fillColor);
            drawRow([unidad.ID_Unidad_Venta, unidad.Nombre], y, false, 'black');
            y += fontSize + cellPadding;
        });
    }
