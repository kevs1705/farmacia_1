...
router.get('/impre_ciudades', async (req, res) => {
    const fecha = obtenerFechaActual();
    const doc = new PDFDocument();

  
    const stream = fs.createWriteStream(`./impresiones/Impre_Ciudades-${fecha}.pdf`);
    ...
    try {
        ...
        // Obtener los datos de las ciudades desde la base de datos
        const dataCiudades = await obtenerDatosCiudades();
        // Llamar a la función para crear la tabla en el PDF
        crearTablaCiudades(doc, dataCiudades);
...
function crearTablaCiudades(doc, dataCiudades) {
    ...
    const tableHeaders = ["ID Ciudad", "Nombre"];
...
function drawTable() {
    ...
    for (let i = 0; i <= dataCiudades.length + 1; i++) {
    ...
    for (let i = 0; i <= tableHeaders.length; i++) {
        doc.moveTo(100 + i * cellWidth, y)
           .lineTo(100 + i * cellWidth, y + (fontSize + cellPadding) * (dataCiudades.length + 1))
           .stroke();
    }
    ...
    dataCiudades.forEach((ciudad, index) => {
        const fillColor = rowColors[index % rowColors.length];
        doc.fillColor(fillColor);
        drawRow([ciudad.ID_Ciudad, ciudad.Nombre], y, false, 'black'); // Dibujar el resto de las filas sin negrita
        y += fontSize + cellPadding;
    });
}
