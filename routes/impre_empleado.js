...
router.get('/impre_empleado', async (req, res) => {
    const fecha = obtenerFechaActual();
    const doc = new PDFDocument();

    const stream = fs.createWriteStream(`./impresiones/Impre_Empleados-${fecha}.pdf`);
    ...
    try {
        ...
        const dataEmpleados = await obtenerDatosEmpleados();
        // Llamar a la función para crear la tabla en el PDF
        crearTablaEmpleados(doc, dataEmpleados);
...
function crearTablaEmpleados(doc, dataEmpleados) {
    ...
    const tableHeaders = ["ID Empleado", "Nombre", "Apellido", "Email", "Teléfono"];
    ...
    function drawTable() {
        ...
        for (let i = 0; i <= dataEmpleados.length + 1; i++) {
        ...
        for (let i = 0; i <= tableHeaders.length; i++) {
            doc.moveTo(100 + i * cellWidth, y)
                .lineTo(100 + i * cellWidth, y + (fontSize + cellPadding) * (dataEmpleados.length + 1))
                .stroke();
        }
        ...
        dataEmpleados.forEach((empleado, index) => {
            const fillColor = rowColors[index % rowColors.length];
            doc.fillColor(fillColor);
            drawRow([empleado.ID_Empleado, empleado.Nombre, empleado.Apellido, empleado.Email, empleado.Teléfono], y, false, 'black'); // Dibujar el resto de las filas sin negrita
            y += fontSize + cellPadding;
        });
    }
